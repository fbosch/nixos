...
- Import to user keyring via `scripts/bootstrap/bootstrap-gpg.sh` for manual editing.
...
1. `./scripts/bootstrap/bootstrap-gpg.sh` — Import GPG key for manual secret editing (optional but recommended)
2. `sudo nixos-rebuild switch --flake .#hostname` — Build system (auto-generates age key)
3. `./scripts/bootstrap/bootstrap-age.sh` — Add age key to `.sops.yaml` and re-encrypt secrets
