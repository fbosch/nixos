...
GTK configuration and Git platform configuration require staged migrations. Git uses Linux Secret Service and macOS Keychain credential helpers. `rvn-srv` uses GPG SSH support, while other hosts use Home Manager `ssh-agent`. `rvn-mac` manages Remote Login and machine login keys; `kmd-mac` does not. The Darwin Podman LaunchAgent discovers and publishes `DOCKER_HOST` for the user session, while Fish derives it only when the session value is absent. Surge hosts define their download root through `services.surge.outputDir`, which its Home Manager service and AppArmor policy share. `rvn-srv` retains the full shared development tool set after its package ownership moves to the system layer. The standalone GTK policy remains an explicit implementation decision.
