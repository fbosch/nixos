...
if [ -n "$staged_shell" ]; then
  if gum spin --spinner dot --title "shfmt" -- sh -c "printf '%s\n' \"\$1\" | xargs -r shfmt -i 2 -s -w" -- "$staged_shell"; then
    printf '%s\n' "$staged_shell" | xargs -r git add
    echo "$(gum style --foreground 2 '[OK]') shfmt (auto-fixed)"
  else
    echo "$(gum style --foreground 1 '[FAIL]') shfmt"
    exit_code=1
  fi

  if gum spin --spinner dot --title "shellcheck" -- sh -c "printf '%s\n' \"\$1\" | xargs -r shellcheck" -- "$staged_shell" >/tmp/shellcheck-output 2>&1; then
