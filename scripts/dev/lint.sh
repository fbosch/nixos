...
if gum spin --spinner dot --title "bootstrap installer" -- bash ./scripts/ci/check-bootstrap-install.sh >/tmp/bootstrap-install-output 2>&1; then
  echo "$(gum style --foreground 2 '[OK]') bootstrap installer"
else
  echo "$(gum style --foreground 1 '[FAIL]') bootstrap installer"
  cat /tmp/bootstrap-install-output
  exit_code=1
fi
