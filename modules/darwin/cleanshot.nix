...
in
{
  flake.modules.darwin.cleanshot =
    { lib, hostMeta, ... }:
    let
      username = if hostMeta.corporate or false then hostMeta.primaryUser or null else personalUsername;
    in
    {
      system.activationScripts.postActivation.text = lib.mkIf (username != null) (
        lib.mkAfter ''
