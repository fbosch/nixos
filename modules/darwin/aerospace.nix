let
  ...
  moveFocusedWindow =
    direction:
    exec "id=$(${aerospace} list-windows --focused --format '%{window-id}') && ${aerospace} move --window-id $id --boundaries all-monitors-outer-frame --boundaries-action stop ${direction} && ${aerospace} focus --window-id $id";
  ...
  moveNodeToWorkspaceOnFocusedMonitor =
    workspace:
    exec "ws=${workspace}; aerospace=${aerospace}; id=$($aerospace list-windows --focused --format '%{window-id}') && ($aerospace list-workspaces --all --format '%{workspace}' | /usr/bin/grep -Fxq \"$ws\" || $aerospace summon-workspace \"$ws\") && $aerospace move-node-to-workspace --focus-follows-window --window-id $id \"$ws\"";
in
{
  flake.modules.darwin.aerospace =
    { lib, hostMeta, ... }:
    {
      config = lib.mkIf (hostMeta.primaryUser or null != null) {
        services.aerospace = {
          enable = true;
          settings = {
