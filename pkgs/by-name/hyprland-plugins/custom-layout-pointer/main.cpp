...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    g_commandEvent = makeShared<Event::CEventBus::CCustomEvent>(
        "custom_layout_pointer.command",
        std::vector{Event::CEventBus::CCustomEvent::TYPE_STRING});

    if (!HyprlandAPI::addEvent(handle, g_commandEvent))
        throw std::runtime_error("custom-layout-pointer: failed to register command event");

    if (!HyprlandAPI::addLuaFunction(handle, "custom_layout_pointer", "start", startResize) ||
        !HyprlandAPI::addLuaFunction(handle, "custom_layout_pointer", "stop", stopResizeLua)) {
